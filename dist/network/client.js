...
function startClient() {
    ...
    const socket = net_1.default.createConnection(PORT, HOST, () => {
        ...
        const versionMessage = (0, message_1.buildMessage)("version", buildVersionPayload());
...
function handleMessage(data, socket) {
    ...
    if (command === "version") {
        ...
        socket.write((0, message_1.buildMessage)("verack", Buffer.alloc(0)));
    }
    if (command === "verack") {
        console.log("Handshake complete ✅");
        socket.write((0, message_1.buildMessage)("getheaders", buildGetHeadersPayload()));
...
function buildGetHeadersPayload() {
    const buffers = [];
    const version = Buffer.alloc(4);
    version.writeInt32LE(70015, 0);
    buffers.push(version);
    ...
    const genesisHash = Buffer.from("000000000933ea01ad0ee984209779baaec3ced90fa3f408719526f8d77f4943", "hex");
    buffers.push(Buffer.from(genesisHash).reverse());
    buffers.push(Buffer.alloc(32)); // stop hash (zero = get max headers)
    return Buffer.concat(buffers);
}
function parseHeadersPayload(payload) {
    let offset = 0;
    ...
    for (let i = 0; i < count; i++) {
        ...
        const { header, hashHex } = (0, chain_1.validateHeader)(headerBuffer);
        if (previousHash) {
